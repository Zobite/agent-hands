 ...
 * Run: cd src/server && bun test src/modules/users/users.test.ts
 */
...
beforeAll(async () => {
  ...
  // Seed superadmin
  const sa = await createSuperAdmin("superadmin", "superadmin@test.local", "password123", "Super Admin");
  superAdminId = sa.id;
  ...
  const adminRes = await app.inject({
    ...
    payload: { username: "testadmin", email: "admin@test.local", password: "password123", name: "Test Admin", role: "admin" },
  });
  ...
  const memberRes = await app.inject({
    ...
    url: "/api/users",
    headers: { ...authHeader(superAdminToken), "content-type": "application/json" },
    payload: { username: "testmember", email: "member@test.local", password: "password123", name: "Test Member" },
  });
...
describe("POST /api/users — Create User", () => {
  test("admin can create user", async () => {
    const res = await app.inject({
      ...
      payload: { username: "newuser1", email: "newuser@test.local", password: "securepass1", name: "New User" },
    });
  ...
  test("admin can create user with specific role", async () => {
    const res = await app.inject({
      ...
      url: "/api/users",
      headers: { ...authHeader(adminToken), "content-type": "application/json" },
      payload: { username: "admin2user", email: "admin2@test.local", password: "securepass1", name: "Admin 2", role: "admin" },
    });

    expect(res.statusCode).toBe(201);
    expect((res.json() as any).role).toBe("admin");
  });
  ...
  test("member cannot create user → 403", async () => {
    const res = await app.inject({
      ...
      payload: { username: "hacker1", email: "hacker@test.local", password: "securepass1", name: "Hacker" },
    });
  ...
  test("no auth → 401", async () => {
    const res = await app.inject({
      ...
      payload: { username: "noauth1", email: "noauth@test.local", password: "securepass1", name: "No Auth" },
    });
  ...
  test("invalid email → 422 validation error", async () => {
    const res = await app.inject({
      ...
      url: "/api/users",
      headers: { ...authHeader(adminToken), "content-type": "application/json" },
      payload: { username: "bademail1", email: "not-an-email", password: "securepass1", name: "Bad Email" },
    });
  ...
  test("password too short → 422 validation error", async () => {
    const res = await app.inject({
      ...
      url: "/api/users",
      headers: { ...authHeader(adminToken), "content-type": "application/json" },
      payload: { username: "shortpw1", email: "short@test.local", password: "short", name: "Short Pass" },
    });
...
describe("GET /api/users — List Users", () => {
  test("admin can list all users", async () => {
    ...
    expect(body.items.length).toBeGreaterThanOrEqual(3); // superadmin + admin + member
...
describe("GET /api/users/:id — Get User", () => {
  test("admin can get any user", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.json() as any).id).toBe(memberId);
  });
  ...
  test("nonexistent user → 400", async () => {
    const res = await app.inject({
      method: "GET",
...
describe("PATCH /api/users/:id — Edit User", () => {
  test("admin can update user name", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.json() as any).name).toBe("Updated Member");
  });
  ...
  test("admin can change user role", async () => {
    ...
    const createRes = await app.inject({
      ...
      url: "/api/users",
      headers: { ...authHeader(adminToken), "content-type": "application/json" },
      payload: { username: "roletest1", email: "roletest@test.local", password: "password123", name: "Role Test" },
    });
    const userId = (createRes.json() as any).id;
  ...
  test("member can edit own profile", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.json() as any).name).toBe("Self Updated");
  });
...
describe("DELETE /api/users/:id — Delete User", () => {
  test("admin can delete user", async () => {
    ...
    const createRes = await app.inject({
      ...
      url: "/api/users",
      headers: { ...authHeader(adminToken), "content-type": "application/json" },
      payload: { username: "todelete1", email: "todelete@test.local", password: "password123", name: "To Delete" },
    });
    const userId = (createRes.json() as any).id;
...
describe("Roles & Permissions", () => {
  test("3 role values are supported: superadmin, admin, member", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const users = (res.json() as any).items;
...
describe("Super Admin Seed", () => {
  ...
  test("cannot create duplicate superadmin", async () => {
    await expect(
      createSuperAdmin("another", "another@test.local", "password123", "Another SA"),
